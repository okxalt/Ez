...
export default function AdminPage() {
  ...
  const { data: admins, mutate: mutateAdmins } = useSWR('/api/admins', fetcher);
  const [email, setEmail] = useState('');
  const [secret, setSecret] = useState('');
  const [minViews, setMinViews] = useState<number>(() => Number(process.env.NEXT_PUBLIC_MIN_VIEWS || 1000));

  const pending = useMemo(() => (submissions || []).filter((s: any) => s.currentViews >= minViews), [submissions, minViews]);
